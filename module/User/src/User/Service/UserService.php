    ...
    UserServiceInterface
{
    ...
    public function setCurrent_user($user) {
    ...
    public function save(array $data, $id = null)
    {
        ...
        if (!$form->isValid()) {
            $this->setMessage('Graag invoer controleren!');
            return false;
        }
        ...
        $usernme = $userdata->offsetGet('login');
        // $this->setMessage($user->getSalutation());
       // Debug::dump($user);
        // check for password
        if ($data['pass'] != '') {
            // encrypt password
        ...
        if ($mode == 'register') {
            ...
            $user->setCreated_by($usernme);
            $user->setCreated_ts(date('Y-m-d H:i:s'));
        }
        else
        {
            $user->setUpdated_by($usernme);
            $user->setUpdated_ts(date('Y-m-d H:i:s'));
        }
     
        // get insert data
        $saveData = $user->getArrayCopy();
       // $this->setMessage($saveData['salutation']);
        ...
        try {
            ...
            $this->setMessage('Gebruiker kan niet opgeslagen worden!');
            return false;
        }
        ...
        // set success message
      //  $this->setMessage('De gegevens zijn opgeslagen!');
    ...
    public function delete($id)
    {
        ...
        // set success message
        $this->setMessage('De gebruiker is verwijderd!');
    ...
    public function login(array $data)
    {
        ...
        // get valid user entity object
        $user = $form->getData();
        $this->setCurrent_user($user['email']);
        $userdata = new Container('userdata');
        $userdata->offsetSet('login',$user['email']);
    ...
    public function logout()
    {
        ...
        // set message
        $this->setMessage('Je bent nu uitgelogd!');
